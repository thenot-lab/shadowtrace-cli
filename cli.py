...
def cmd_research(args: argparse.Namespace) -> int:
    ...
    print(f"subject_id: {meta['subject_id']}  tier: {meta['consent']['tier']}  eu: {meta['consent']['eu_subject']}")
    ...
    if prof.get("top_locations"):
        print("LOCATIONS:  " + ", ".join(f"{l} ({c})" for l, c in prof["top_locations"]))
    ...
    if chains:
        ...
        for ch in chains[:5]:
            print(f"  [{ch['confidence']:.2f}] len={ch['length']} sources={','.join(ch['sources'])}")
    ...
    if not fp.get('insufficient_data'):
        ...
        print(f"BEHAVIORAL FINGERPRINT  wc={fp.get('word_count',0)} sents={fp.get('sentence_count',0)}")
        print(f"  lexical: avg_sent_len={lex.get('avg_sentence_len')} ttr={lex.get('type_token_ratio')}")
        print(f"  stylistic: hedge/1k={sty.get('hedge_rate_per_1k')} intens/1k={sty.get('intensifier_rate_per_1k')}")
...
def cmd_report(args: argparse.Namespace) -> int:
    ...
    if not p.exists():
        print(f"No cached report for {args.subject_id}"); return 1
...
def cmd_audit(args: argparse.Namespace) -> int:
    from .audit import source_log, rate_limit
...
def main(argv=None) -> int:
    ...
    r.add_argument("--consented", action="store_true", help="subject gave explicit consent (deep tier)")
    ...
    e = sub.add_parser("enrich"); e.add_argument("identifier")
    rep = sub.add_parser("report"); rep.add_argument("subject_id")
    rc = sub.add_parser("recompile"); rc.add_argument("--since", default=None)
    au = sub.add_parser("audit"); au.add_argument("--tail", type=int, default=20)

    args = p.parse_args(argv)
    fn = {
        "research": cmd_research, "enrich": cmd_enrich,
        "report": cmd_report, "recompile": cmd_recompile, "audit": cmd_audit,
    }[args.cmd]
    return fn(args)
